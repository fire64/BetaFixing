push @output, "vs.1.1																								; LINEINFO(Teeth.vsh)(1)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(2)\n";
 	 push @output, ";	 c90	 = xyz = mouth forward direction vector, w = illum factor																								; LINEINFO(Teeth.vsh)(3)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(4)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(8)\n";
push @output, "; Vertex blending 																								; LINEINFO(Teeth.vsh)(9)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(10)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(15)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Teeth.vsh)(16)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(17)\n";
local( $projPos ); &AllocateRegister( \$projPos, "\$projPos" );
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(Teeth.vsh)(21)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(Teeth.vsh)(22)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(Teeth.vsh)(23)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(Teeth.vsh)(24)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(Teeth.vsh)(25)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(27)\n";
push @output, "; Fog																								; LINEINFO(Teeth.vsh)(28)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(29)\n";
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$projPos, "\$projPos" );
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(35)\n";
push @output, "; Lighting																								; LINEINFO(Teeth.vsh)(36)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(37)\n";
local( $linearColor ); &AllocateRegister( \$linearColor, "\$linearColor" );
&DoDynamicLightingToLinear( $g_ambientLightType, $g_localLightType1, $g_localLightType2, 																								
                            $worldPos, $worldNormal, $linearColor );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(42)\n";
push @output, "; Factor in teeth darkening factors																								; LINEINFO(Teeth.vsh)(43)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(44)\n";
local( $tmp ); &AllocateRegister( \$tmp, "\$tmp" );
push @output, "mul $linearColor.xyz, c90.w, $linearColor	; FIXME Color darkened by illumination factor																								; LINEINFO(Teeth.vsh)(48)\n";
push @output, "dp3 $tmp, $worldNormal, c90					; Figure out mouth forward dot normal																								; LINEINFO(Teeth.vsh)(49)\n";
push @output, "max	$tmp, $cZero, $tmp						; clamp from 0 to 1																								; LINEINFO(Teeth.vsh)(50)\n";
push @output, "mul $linearColor.xyz, $tmp, $linearColor	; Darken by forward dot normal too																								; LINEINFO(Teeth.vsh)(51)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(53)\n";
push @output, "; Output color (gamma correction)																								; LINEINFO(Teeth.vsh)(54)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(55)\n";
local( $gammaColor ); &AllocateRegister( \$gammaColor, "\$gammaColor" );
&LinearToGamma( $linearColor, $gammaColor );																								
&FreeRegister( \$linearColor, "\$linearColor" );
push @output, "mul oD0.xyz, $gammaColor.xyz, $cOverbrightFactor																								; LINEINFO(Teeth.vsh)(60)\n";
push @output, "mov oD0.w, c0.y				; make sure all components are defined																								; LINEINFO(Teeth.vsh)(61)\n";
&FreeRegister( \$gammaColor, "\$gammaColor" );
&FreeRegister( \$worldPos, "\$worldPos" );
&FreeRegister( \$worldNormal, "\$worldNormal" );
&FreeRegister( \$tmp, "\$tmp" );
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(69)\n";
push @output, "; Texture coordinates																								; LINEINFO(Teeth.vsh)(70)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(71)\n";
push @output, "mov oT0, $vTexCoord0																								; LINEINFO(Teeth.vsh)(73)\n";
