push @output, "vs.1.1																								; LINEINFO(glowwarp.vsh)(1)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(5)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(7)\n";
push @output, "; Vertex blending 																								; LINEINFO(glowwarp.vsh)(8)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(9)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(14)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(glowwarp.vsh)(15)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(16)\n";
&AllocateRegister( \$projPos, "\$projPos" );																								
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(glowwarp.vsh)(20)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(glowwarp.vsh)(21)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(glowwarp.vsh)(22)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(glowwarp.vsh)(23)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(glowwarp.vsh)(24)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(26)\n";
push @output, "; Transform the normal from world to view space																								; LINEINFO(glowwarp.vsh)(27)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(28)\n";
&AllocateRegister( \$projNormal, "\$projNormal" );																								
push @output, "; only do X and Y since that's all we care about																								; LINEINFO(glowwarp.vsh)(32)\n";
push @output, "dp3 $projNormal.x, $worldNormal, $cViewProj0																								; LINEINFO(glowwarp.vsh)(33)\n";
push @output, "dp3 $projNormal.y, $worldNormal, $cViewProj1																								; LINEINFO(glowwarp.vsh)(34)\n";
push @output, "dp3 $projNormal.z, $worldNormal, $cViewProj2																								; LINEINFO(glowwarp.vsh)(35)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(37)\n";
push @output, "; Fog																								; LINEINFO(glowwarp.vsh)(38)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(39)\n";
&CalcFog( $worldPos, $projPos );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(42)\n";
push @output, "; Texture coordinates																								; LINEINFO(glowwarp.vsh)(43)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(glowwarp.vsh)(44)\n";
push @output, "; divide by z																								; LINEINFO(glowwarp.vsh)(46)\n";
push @output, "rcp $worldPos.w, $worldPos.w																								; LINEINFO(glowwarp.vsh)(47)\n";
push @output, "mul $worldPos.xy, $worldPos.w, $worldPos.xy																								; LINEINFO(glowwarp.vsh)(48)\n";
push @output, "; map from -1..1 to 0..1																								; LINEINFO(glowwarp.vsh)(50)\n";
push @output, "mad $worldPos.xy, $worldPos.xy, $cHalf, $cHalf																								; LINEINFO(glowwarp.vsh)(51)\n";
push @output, "; x *= 1																								; LINEINFO(glowwarp.vsh)(53)\n";
push @output, "; y *= 3/4																								; LINEINFO(glowwarp.vsh)(54)\n";
push @output, "mul $worldPos.xy, $worldPos.xy, c93.xy ; FIXME																								; LINEINFO(glowwarp.vsh)(55)\n";
push @output, "; tweak the texcoord offset by alpha																								; LINEINFO(glowwarp.vsh)(57)\n";
push @output, "mul $projNormal.xy, $projNormal.xy, $vColor.w																								; LINEINFO(glowwarp.vsh)(58)\n";
push @output, "; tweak with the texcoords based on the normal and $basetextureoffset (need to rename)																								; LINEINFO(glowwarp.vsh)(60)\n";
push @output, "mad $worldPos.xy, $projNormal.xy, -c94.xy, $worldPos.xy ; FIXME																								; LINEINFO(glowwarp.vsh)(61)\n";
push @output, "; y = .75-y																								; LINEINFO(glowwarp.vsh)(63)\n";
push @output, "add $worldPos.y, c95.x, -$worldPos.y ; FIXME																								; LINEINFO(glowwarp.vsh)(64)\n";
push @output, "; clamp the texcoords																								; LINEINFO(glowwarp.vsh)(66)\n";
push @output, "min $worldPos.y, c95.w, $worldPos.y ; FIXME																								; LINEINFO(glowwarp.vsh)(67)\n";
push @output, "mov oT0.xy, $worldPos.xy																								; LINEINFO(glowwarp.vsh)(69)\n";
push @output, "mov oD0, $cConstants0.wyyw																								; LINEINFO(glowwarp.vsh)(71)\n";
