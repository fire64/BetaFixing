push @output, "vs.1.1																								; LINEINFO(Refract_model_vs11.vsh)(1)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(2)\n";
push @output, "; Constants specified by the app																								; LINEINFO(Refract_model_vs11.vsh)(3)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(Refract_model_vs11.vsh)(4)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(Refract_model_vs11.vsh)(5)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(Refract_model_vs11.vsh)(6)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(Refract_model_vs11.vsh)(7)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(Refract_model_vs11.vsh)(8)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(Refract_model_vs11.vsh)(9)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(Refract_model_vs11.vsh)(10)\n";
 																								push @output, ";																								; LINEINFO(Refract_model_vs11.vsh)(11)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(Refract_model_vs11.vsh)(12)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(Refract_model_vs11.vsh)(13)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(Refract_model_vs11.vsh)(14)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(15)\n";
...
push @output, "; Vertex components																								; LINEINFO(Refract_model_vs11.vsh)(19)\n";
     push @output, ";    $vPos		= Position																								; LINEINFO(Refract_model_vs11.vsh)(20)\n";
 	 push @output, ";	 $vNormal		= normal																								; LINEINFO(Refract_model_vs11.vsh)(21)\n";
 	 push @output, ";	 $vTexCoord0.xy	= TexCoord0																								; LINEINFO(Refract_model_vs11.vsh)(22)\n";
 	 push @output, ";	 $vTangentS		= S axis of Texture space																								; LINEINFO(Refract_model_vs11.vsh)(23)\n";
 	 push @output, ";	 $vTangentT	= T axis of Texture space																								; LINEINFO(Refract_model_vs11.vsh)(24)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(26)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Refract_model_vs11.vsh)(27)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(28)\n";
...
push @output, "; Transform position from world to projection space																								; LINEINFO(Refract_model_vs11.vsh)(40)\n";
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(Refract_model_vs11.vsh)(41)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(Refract_model_vs11.vsh)(42)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(Refract_model_vs11.vsh)(43)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(Refract_model_vs11.vsh)(44)\n";
if( $g_fogType eq "rangefog" )																								
{																								
	&RangeFog( $projPos );																								
}																								
...
push @output, "; Get the eye vector in world space																								; LINEINFO(Refract_model_vs11.vsh)(61)\n";
push @output, "add $worldEyeVect.xyz, -$worldPos, $cEyePos																								; LINEINFO(Refract_model_vs11.vsh)(62)\n";
local( $tangentEyeVect ); &AllocateRegister( \$tangentEyeVect, "\$tangentEyeVect" );
push @output, "; transform the eye vector to tangent space																								; LINEINFO(Refract_model_vs11.vsh)(65)\n";
push @output, "dp3 oT3.x, $worldEyeVect, $worldTangentS																								; LINEINFO(Refract_model_vs11.vsh)(66)\n";
push @output, "dp3 oT3.y, $worldEyeVect, $worldTangentT																								; LINEINFO(Refract_model_vs11.vsh)(67)\n";
push @output, "dp3 oT3.z, $worldEyeVect, $worldNormal																								; LINEINFO(Refract_model_vs11.vsh)(68)\n";
local( $bumpTexCoord ); &AllocateRegister( \$bumpTexCoord, "\$bumpTexCoord" );
push @output, "dp4 $bumpTexCoord.x, $vTexCoord0, c91																								; LINEINFO(Refract_model_vs11.vsh)(72)\n";
push @output, "dp4 $bumpTexCoord.y, $vTexCoord0, c92																								; LINEINFO(Refract_model_vs11.vsh)(73)\n";
push @output, "; dudv map																								; LINEINFO(Refract_model_vs11.vsh)(75)\n";
push @output, "mov oT0.xy, $bumpTexCoord																								; LINEINFO(Refract_model_vs11.vsh)(76)\n";
push @output, "; refract tint																								; LINEINFO(Refract_model_vs11.vsh)(78)\n";
push @output, "mov oT2.xy, $bumpTexCoord																								; LINEINFO(Refract_model_vs11.vsh)(79)\n";
&FreeRegister( \$bumpTexCoord, "\$bumpTexCoord" );
push @output, "mov oPos, $projPos																								; LINEINFO(Refract_model_vs11.vsh)(83)\n";
push @output, "; special case perspective correct texture projection so that the texture fits exactly on the screen																								; LINEINFO(Refract_model_vs11.vsh)(85)\n";
push @output, "mul $projPos.y, $projPos.y, c94.w																								; LINEINFO(Refract_model_vs11.vsh)(86)\n";
push @output, "add $projPos.xy, $projPos.xy, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(87)\n";
push @output, "mul $projPos.xy, $projPos.xy, $cHalf																								; LINEINFO(Refract_model_vs11.vsh)(88)\n";
push @output, "mov oT1.xy, $projPos.xy																								; LINEINFO(Refract_model_vs11.vsh)(90)\n";
push @output, "; emit w to both z and w in case the driver screws up and divides by z																								; LINEINFO(Refract_model_vs11.vsh)(91)\n";
push @output, "mov oT1.z, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(92)\n";
push @output, "mov oT1.w, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(93)\n";
