push @output, "vs.1.1																								; LINEINFO(WaterCheap_vs11.vsh)(1)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(5)\n";
push @output, "; Vertex blending																								; LINEINFO(WaterCheap_vs11.vsh)(6)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(7)\n";
&AllocateRegister( \$worldPos, "\$worldPos" );																								
push @output, "; Transform position from object to world																								; LINEINFO(WaterCheap_vs11.vsh)(11)\n";
push @output, "dp4 $worldPos.x, $vPos, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(12)\n";
push @output, "dp4 $worldPos.y, $vPos, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(13)\n";
push @output, "dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(14)\n";
&AllocateRegister( \$projPos, "\$projPos" );																								
push @output, "; Transform position from object to projection space																								; LINEINFO(WaterCheap_vs11.vsh)(18)\n";
push @output, "dp4 $projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(WaterCheap_vs11.vsh)(19)\n";
push @output, "dp4 $projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(WaterCheap_vs11.vsh)(20)\n";
push @output, "dp4 $projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(WaterCheap_vs11.vsh)(21)\n";
push @output, "dp4 $projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(WaterCheap_vs11.vsh)(22)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(WaterCheap_vs11.vsh)(24)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(26)\n";
push @output, "; Fog																								; LINEINFO(WaterCheap_vs11.vsh)(27)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(28)\n";
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$projPos, "\$projPos" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(33)\n";
push @output, "; Lighting																								; LINEINFO(WaterCheap_vs11.vsh)(34)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(35)\n";
push @output, "; Transform tangent space basis vectors to env map space (world space)																								; LINEINFO(WaterCheap_vs11.vsh)(37)\n";
push @output, "; This will produce a set of vectors mapping from tangent space to env space																								; LINEINFO(WaterCheap_vs11.vsh)(38)\n";
push @output, "; We'll use this to transform normals from the normal map from tangent space																								; LINEINFO(WaterCheap_vs11.vsh)(39)\n";
push @output, "; to environment map space. 																								; LINEINFO(WaterCheap_vs11.vsh)(40)\n";
push @output, "; NOTE: use dp3 here since the basis vectors are vectors, not points																								; LINEINFO(WaterCheap_vs11.vsh)(41)\n";
push @output, "dp3 oT1.x, $vTangentS, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(43)\n";
push @output, "dp3 oT2.x, $vTangentS, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(44)\n";
push @output, "dp3 oT3.x, $vTangentS, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(45)\n";
push @output, "dp3 oT1.y, $vTangentT, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(47)\n";
push @output, "dp3 oT2.y, $vTangentT, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(48)\n";
push @output, "dp3 oT3.y, $vTangentT, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(49)\n";
push @output, "dp3 oT1.z, $vNormal, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(51)\n";
push @output, "dp3 oT2.z, $vNormal, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(52)\n";
push @output, "dp3 oT3.z, $vNormal, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(53)\n";
push @output, "; Compute the vector from vertex to camera																								; LINEINFO(WaterCheap_vs11.vsh)(55)\n";
&AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );																								
push @output, "sub $worldEyeVect.xyz, $cEyePos, $worldPos																								; LINEINFO(WaterCheap_vs11.vsh)(57)\n";
&FreeRegister( \$worldPos, "\$worldPos" );																								
push @output, "; Move it into the w component of the texture coords, as the wacky																								; LINEINFO(WaterCheap_vs11.vsh)(60)\n";
push @output, "; pixel shader wants it there.																								; LINEINFO(WaterCheap_vs11.vsh)(61)\n";
push @output, "mov oT1.w, $worldEyeVect.x																								; LINEINFO(WaterCheap_vs11.vsh)(62)\n";
push @output, "mov oT2.w, $worldEyeVect.y																								; LINEINFO(WaterCheap_vs11.vsh)(63)\n";
push @output, "mov oT3.w, $worldEyeVect.z																								; LINEINFO(WaterCheap_vs11.vsh)(64)\n";
local( $tangentEyeVect ); &AllocateRegister( \$tangentEyeVect, "\$tangentEyeVect" );
push @output, "; transform the eye vector to tangent space																								; LINEINFO(WaterCheap_vs11.vsh)(68)\n";
push @output, "dp3 $tangentEyeVect.x, $worldEyeVect, $vTangentS																								; LINEINFO(WaterCheap_vs11.vsh)(69)\n";
push @output, "dp3 $tangentEyeVect.y, $worldEyeVect, $vTangentT																								; LINEINFO(WaterCheap_vs11.vsh)(70)\n";
push @output, "dp3 $tangentEyeVect.z, $worldEyeVect, $vNormal																								; LINEINFO(WaterCheap_vs11.vsh)(71)\n";
push @output, "; Get the magnitude of worldEyeVect																								; LINEINFO(WaterCheap_vs11.vsh)(73)\n";
push @output, "dp3 $worldEyeVect.w, $worldEyeVect, $worldEyeVect																								; LINEINFO(WaterCheap_vs11.vsh)(74)\n";
push @output, "rsq $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(WaterCheap_vs11.vsh)(75)\n";
push @output, "rcp $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(WaterCheap_vs11.vsh)(76)\n";
push @output, "; calculate the cheap water blend factor and stick it into oD0.a																								; LINEINFO(WaterCheap_vs11.vsh)(78)\n";
push @output, "; NOTE: This won't be perspective correct!!!!!																								; LINEINFO(WaterCheap_vs11.vsh)(79)\n";
push @output, "; OPTIMIZE: This could turn into a mad.																								; LINEINFO(WaterCheap_vs11.vsh)(80)\n";
push @output, "add $worldEyeVect.w, $worldEyeVect.w, -c92.x																								; LINEINFO(WaterCheap_vs11.vsh)(81)\n";
push @output, "mul $worldEyeVect.w, $worldEyeVect.w, c92.y																								; LINEINFO(WaterCheap_vs11.vsh)(82)\n";
push @output, ";max $worldEyeVect.w, $worldEyeVect.w, $cZero																								; LINEINFO(WaterCheap_vs11.vsh)(83)\n";
push @output, ";min oD0.w, $worldEyeVect.w, $cOne																								; LINEINFO(WaterCheap_vs11.vsh)(84)\n";
&Normalize( $tangentEyeVect );																								
push @output, "; stick the tangent space eye vector into oD0.xyz																								; LINEINFO(WaterCheap_vs11.vsh)(89)\n";
push @output, "mad oD0.xyz, $tangentEyeVect, $cHalf, $cHalf																								; LINEINFO(WaterCheap_vs11.vsh)(90)\n";
push @output, "; stick the magnitude of the eye vector in oD0.w																								; LINEINFO(WaterCheap_vs11.vsh)(92)\n";
push @output, "mov oD0.w, $worldEyeVect.w																								; LINEINFO(WaterCheap_vs11.vsh)(93)\n";
&FreeRegister( \$worldEyeVect, "\$worldEyeVect" );																								
&FreeRegister( \$tangentEyeVect, "\$tangentEyeVect" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(98)\n";
push @output, "; Texture coordinates																								; LINEINFO(WaterCheap_vs11.vsh)(99)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(100)\n";
push @output, "dp4 oT0.x, $vTexCoord0, c90																								; LINEINFO(WaterCheap_vs11.vsh)(101)\n";
push @output, "dp4 oT0.y, $vTexCoord0, c91																								; LINEINFO(WaterCheap_vs11.vsh)(102)\n";
