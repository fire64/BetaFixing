push @output, "vs.1.1																								; LINEINFO(WorldTexture.vsh)(1)\n";
...
push @output, "dp4 $projPos.x, $vPos, $cViewProj0																								; LINEINFO(WorldTexture.vsh)(7)\n";
push @output, "dp4 $projPos.y, $vPos, $cViewProj1																								; LINEINFO(WorldTexture.vsh)(8)\n";
push @output, "dp4 $projPos.z, $vPos, $cViewProj2																								; LINEINFO(WorldTexture.vsh)(9)\n";
push @output, "dp4 $projPos.w, $vPos, $cViewProj3																								; LINEINFO(WorldTexture.vsh)(10)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(WorldTexture.vsh)(11)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WorldTexture.vsh)(13)\n";
push @output, "; Fog																								; LINEINFO(WorldTexture.vsh)(14)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WorldTexture.vsh)(15)\n";
local( $worldPos ); &AllocateRegister( \$worldPos, "\$worldPos" );
if( $g_fogType eq "heightfog" )																								
{																								
	push @output, "	; Get the worldpos z component only since that's all we need for height fog																								; LINEINFO(WorldTexture.vsh)(19)\n";
	push @output, "	dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(WorldTexture.vsh)(20)\n";
}																								
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$worldPos, "\$worldPos" );
&FreeRegister( \$projPos, "\$projPos" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WorldTexture.vsh)(27)\n";
push @output, "; Texture coordinates																								; LINEINFO(WorldTexture.vsh)(28)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WorldTexture.vsh)(29)\n";
push @output, "; Just copy the texture coordinates	for the texture																								; LINEINFO(WorldTexture.vsh)(31)\n";
push @output, "mov oT0, $vTexCoord0																								; LINEINFO(WorldTexture.vsh)(32)\n";
